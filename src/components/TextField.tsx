...
export interface TextFieldProps extends InputProps, VariantProps<typeof textField> {
  label: string
  helperText?: string
...
export const TextField = forwardRef<HTMLInputElement, TextFieldProps>(
  (
    { label, id, className = "", helperText, status = "default", helperTextProps, startContent, endContent, ...props },
    ref,
  ) => {
      ...
      <div className={textField({ status, className })}>
        <div data-testid={"test"} className={inputWrapper({ status })}>
          <div className={contentWrapper({ status })}>
            {startContent && <div className={contentStyles()}>{startContent}</div>}
        ...
        {helperText && (
          <HelperText id={`${id}-helper-text`} status={status} {...helperTextProps}>
            {helperText}
          </HelperText>
        )}
      </div>
